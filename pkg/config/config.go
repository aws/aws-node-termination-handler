...
const (
	...
	drainTimeBeforeTerminationKey      = "DRAIN_TIME_BEFORE_TERMINATION"
	drainTimeBeforeTerminationDefault  = 120
)
...
type Config struct {
	...
	DrainTimeBeforeTermination int
}
...
func ParseCliArgs() Config {
	...
	flag.IntVar(&config.DrainTimeBeforeTermination, "drain-time-before-termination", getIntEnv(drainTimeBeforeTerminationKey, drainTimeBeforeTerminationDefault), "The number of seconds to drain the node before termination")
	...
	fmt.Printf("aws-node-termination-handler arguments: \n"+
		...
		"\tgrace-period: %d\n"+
		"\tdrain-time-before-termination: %d\n",
		config.DryRun,
		config.NodeName,
		config.MetadataURL,
		...
		config.DrainTimeBeforeTermination)
