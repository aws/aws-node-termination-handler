...
func CheckForSpotInterruptionNotice(metadataURL string, drainTimeBeforeTermination int) bool {
	...
	if timeUntilInterruption <= (time.Duration(drainTimeBeforeTermination) * time.Second) {
		return true
	}
	log.Printf("Termination notice received, but the time until interruption is %d and the node is configured to drain for %d sec. Waiting to drain...", timeUntilInterruption, drainTimeBeforeTermination)
	return false
}
